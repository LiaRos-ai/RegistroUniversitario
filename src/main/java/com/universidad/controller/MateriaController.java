...
public class MateriaController {
    ...
    //-------------------------------------
    @Transactional(readOnly = false) // Esta línea debes añadir
    @PutMapping("/{id}/unidades")
    public ResponseEntity<?> reemplazarUnidades(@PathVariable Long id, @RequestBody List<UnidadTematica> nuevasUnidades) {
    ...
    public ResponseEntity<List<UnidadTematica>> getUnidadesPorMateria(@PathVariable Long id) {
        return materiaRepository.findMateriaById(id)
                .map(m -> ResponseEntity.ok(m.getUnidadesTematicas()))
                .orElse(ResponseEntity.notFound().build());
    }
