...
public class MateriaController {
    ...
    public ResponseEntity<List<UnidadTematicaDTO>> obtenerUnidadesDeMateria(@PathVariable Long id) {
        long inicio = System.currentTimeMillis();
        logger.info("[UNIDAD] Inicio obtenerUnidadesDeMateria: {}", inicio);
        List<UnidadTematicaDTO> unidades = unidadTematicaService.obtenerUnidadesPorMateriaId(id);
        long fin = System.currentTimeMillis();
        logger.info("[UNIDAD] Fin obtenerUnidadesDeMateria: {} (Duracion: {} ms)", fin, (fin - inicio));
